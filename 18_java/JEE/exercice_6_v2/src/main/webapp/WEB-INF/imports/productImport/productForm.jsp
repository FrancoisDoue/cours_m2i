...
<jsp:useBean id="currentProduct" type="exercice_6.entity.Product" scope="request" />

<h4 class="px-4 pb-4 border-bottom border-light text-bold"><%= (currentProduct.getId() > 0) ? "Modifier" : "Ajouter" %> un produit</h4>
<div class="w-50 m-auto">

    <form method="post"
          id="main-form"
          enctype="multipart/form-data"
          action="${pageContext.request.contextPath}/products<%= (currentProduct.getId() > 0) ? "?id=" + currentProduct.getId() : "" %>">
        ...
        <div class="d-flex flex-column px-2 my-4">
            ...
            <div class="input-group mb-3">
                <input class="form-control bg-dark border border-light text-light" type="date" name="buyDate" id="buyDate" required
                        value="<%= (currentProduct.getId() > 0) ? currentProduct.getBuyAt() : LocalDate.now() %>"/>
            </div>
        </div>
<%--            <div class="d-flex flex-column px-2 my-4">--%>
...
<%--            </div>--%>
        <div class="py-4 border-top border-light">
            <button class="btn btn-light w-100" id="submit-form" >
                <%= (currentProduct.getId() > 0) ? "Modifier" : "Ajouter" %>
            </button>
        </div>
    </form>
</div>
